...
class LinearChainCRF(Learner):
    ...
    * ``'targets'``:    Set of possible targets.
    ...
    def fprop(self,input,target):
        ...
        auyk,apykyk1=0,0#auyk has dimensions [input_size,n_classes]
        for i in range(len(input)):
            #  self.bias = np.zeros((self.n_classes))            
            if i>0 & i<len(input):
                auyk=np.dot(self.weights[0]*input[i])+np.dot(self.weights[1]*input[i+1])+np.dot(self.weights[0]*input[i-1])+self.bias
            elif i>0:#i==last one
                auyk=np.dot(self.weights[0]*input[i])+np.dot(self.weights[0]*input[i-1])+self.bias
            else:#i==0
                auyk=np.dot(self.weights[0]*input[i])+np.dot(self.weights[1]*input[i+1])+self.bias
            auyk+=auyk
           
            apykyk1=self.lateral_weights(target,target+1)
